  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--grad-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'var(--shadow-primary)' }}>
            <Icon name="favorite" size={18} color="#fff" fill={1} />
          </div>
          <div>
      ...
      <nav style={{ flex: 1, padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {ADMIN_NAV.map(item => {
          ...
          return (
            ...
            >
              <Icon name={item.icon} size={20} fill={isActive ? 1 : 0} color={isActive ? 'var(--primary)' : 'var(--text-secondary)'} />
              {item.label}
            </button>
          );
        })}
      ...
      <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <Icon name="logout" size={18} color="var(--text-hint)" style={{ cursor: 'pointer', flexShrink: 0 }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, color: 'var(--text-secondary)' }}>
      <div style={{ width: 60, height: 60, borderRadius: 'var(--r-2xl)', background: 'var(--primary-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Icon name={icon} size={28} color="var(--primary)" fill={1} />
      </div>
...
export function AdminApp() {
  ...
  else if (screen === 'appointments') content = <PlaceholderScreen title="Appointments" icon="calendar_today" />;
  else if (screen === 'subscriptions') content = <PlaceholderScreen title="Subscriptions" icon="workspace_premium" />;
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'var(--font-sans)', background: 'var(--bg)', color: 'var(--text-primary)' }}>
