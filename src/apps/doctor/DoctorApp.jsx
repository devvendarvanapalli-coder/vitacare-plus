      ...
      {DOC_NAV.map((item, i) => {
        ...
        return (
          <button key={i} onClick={() => onTab(i)}
            ...
            <Icon name={item.icon} size={24} fill={active ? 1 : 0} color={active ? 'var(--primary)' : 'var(--text-hint)'} />
...
export function DoctorApp() {
  ...
  const deviceContent = push ? (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      <div style={{ height: 50, flexShrink: 0 }} />
      {push.screen === 'consultation' ? (
        <ConsultationScreen
          patient={push.data}
          onBack={() => setPush(null)}
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
        {tab === 0 && <ScheduleScreen onOpenConsultation={d => setPush({ screen: 'consultation', data: d })} />}
        {tab === 1 && <PatientsScreen onOpenConsultation={d => setPush({ screen: 'consultation', data: d })} />}
        {tab === 2 && <RxListTab onNew={() => setPush({ screen: 'rx', data: {} })} />}
        {tab === 3 && <ProfileTab />}
      </div>
