...
function VitalTile({ label, value, unit, icon, color }) {
  return (
    <div style={{ flex: 1, background: 'var(--card)', borderRadius: 'var(--r-lg)', padding: '10px 10px 12px', boxShadow: 'var(--shadow-card)', display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ width: 30, height: 30, borderRadius: 'var(--r-md)', background: `color-mix(in srgb, ${color} 12%, transparent)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Icon name={icon} size={16} color={color} />
      </div>
      <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: -0.5, marginTop: 4 }}>{value}</div>
      <div style={{ fontSize: 9, color: 'var(--text-secondary)', fontWeight: 600 }}>{unit}</div>
      <div style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{label}</div>
    </div>
  );
}

export function ConsultationScreen({ patient = {}, onBack, onWriteRx }) {
            ...
            {RECENT_RX.map(rx => (
              <Card key={rx.name} padding={12} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 34, height: 34, borderRadius: 'var(--r-md)', background: 'color-mix(in srgb, var(--primary) 10%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon name="medication" size={18} color="var(--primary)" fill={1} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{rx.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>{rx.sig}</div>
                </div>
